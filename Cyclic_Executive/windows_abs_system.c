...
#define RESPONSE_TIME_MS 99  // Tolerância de 1ms para o atraso do fio
#define EXECUTION_PERIOD_MS 200  // Período de execução cíclica
...
static void cyclic_executive_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        vTaskDelayUntil(&last_wake_time, EXECUTION_PERIOD_MS / portTICK_PERIOD_MS);
    }
}
...
#endif
    // Iniciar a tarefa cíclica
    xTaskCreate(&cyclic_executive_task, "cyclic_executive_task", 2048, NULL, 5, NULL);
}
